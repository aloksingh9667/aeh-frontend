...
export default function StudentDashboard() {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center"><div className="animate-spin h-8 w-8 border-4 border-brand-primary border-t-transparent rounded-full" /></div>;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8">
        <div className="bg-brand-primary rounded-2xl p-6 sm:p-8 text-white mb-6">
...
function ActionCard({ href, icon, title, description, color }: { href: string; icon: React.ReactNode; title: string; description: string; color: string }) {
  ...
  return (
    <Link href={href} className="group bg-white rounded-xl border border-gray-200 p-6 hover:border-brand-primary hover:shadow-md transition-all block">
      <div className={`w-14 h-14 rounded-xl ${colors[color]} flex items-center justify-center mb-4 transition-colors`}>{icon}</div>
      <h3 className="font-semibold text-gray-800 mb-1 group-hover:text-[var(--brand-primary)]">{title}</h3>
