...
function generateReceiptHTML(p: Payment, studentName: string, rollNumber: string, course: string, institutionName = "Our College", contactLine = ""): string {
  ...
  <div class="page">
    ...
    <div class="footer">
      <div class="footer-note">
        ...
        Retain this receipt for future reference. | For payment queries, contact the accounts office.<br/>
      ...
      <div class="watermark">
        <span>Generated by ${institutionName} Student Portal • ${new Date().toLocaleString("en-IN")}</span>
      </div>
    </div>
...
export default function StudentReceipts() {
  ...
  const institutionName = config?.collegeName || "Our College";
  ...
  const openReceipt = (p: Payment) => {
    const html = generateReceiptHTML(p, student?.name || p.studentName || "N/A", student?.rollNumber || p.rollNumber || "N/A", student?.course || p.courseName || "N/A", institutionName, contactLine);
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <a href="/student/fees" className="inline-block mt-4 bg-brand-primary text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:opacity-80 transition-colors">Pay Fees Now</a>
          </div>
        ) : (
          <div className="space-y-3">
            {payments.map(p => (
                    ...
                    {p.status === "success" && (
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-sm text-brand-primary border border-brand-primary px-3 py-1.5 rounded-lg hover:bg-brand-primary hover:text-white transition-colors"
                        >
                          <Eye className="h-4 w-4" /> View
                        </button>
                        <button
                          onClick={() => openReceipt(p)}
                          className="flex items-center gap-1.5 text-sm bg-brand-primary text-white px-3 py-1.5 rounded-lg hover:opacity-80 transition-colors"
                        >
