    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-gradient-to-br from-brand-primary to-brand-primary text-white py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-white/70 text-lg">The governing body overseeing academic excellence at {collegeName}</p>
        </div>
      </div>
      <main className="flex-1 max-w-5xl mx-auto px-4 py-12 w-full">
        <p className="text-muted-foreground text-lg mb-10 leading-relaxed">
          The Academic Council of {collegeName} is the apex body responsible for curriculum development, academic policy, quality assurance, and maintaining the highest standards of education across all programs.
        </p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {members.map(({ name, role, dept }) => (
            <div key={name} className="bg-card border border-border rounded-xl p-5 flex gap-4">
              <div className="h-12 w-12 bg-brand-primary text-white rounded-xl flex items-center justify-center text-lg font-bold shrink-0">
                {name.split(" ").filter(w => w !== "Dr." && w !== "Prof." && w !== "Shri")[0][0]}
              </div>
              <div>
                <h3 className="font-semibold text-foreground text-sm">{name}</h3>
                <p className="text-brand-primary text-xs font-medium mt-0.5">{role}</p>
