...
export default function Gallery() {
  ...
  const sections = (config?.gallery && config.gallery.length > 0)
    ? config.gallery.map(s => ({
        title: s.title,
        desc: s.description ?? "",
        items: s.items.filter(it => it.imageUrl?.trim()).map(it => ({ label: it.label, img: it.imageUrl })),
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-gradient-to-br from-brand-primary to-brand-primary text-white py-14 px-4">
