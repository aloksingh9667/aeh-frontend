...
import { z } from "zod";
import { useCreateApplication } from "@/lib/api-client/index";
...
export default function Apply() {
  ...
  const cfgPhone = config?.phone || "+91 9876543210";
  const cfgEmail = config?.email || "admissions@college.edu.in";
  const cfgLocation = [config?.city, config?.state].filter(Boolean).join(", ") || "Our Campus";
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-gradient-to-br from-brand-primary to-brand-primary text-white py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-white/70 text-lg">Begin your journey at {collegeName}</p>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            {submitted ? (
              <div className="bg-green-50 border border-green-200 rounded-2xl p-10 text-center">
                ...
                <p className="text-muted-foreground mb-6">Thank you for applying to {collegeName}. Our admissions team will contact you within 24-48 hours.</p>
                <button onClick={() => setSubmitted(false)} className="bg-brand-primary text-white font-semibold px-6 py-2.5 rounded-lg hover:opacity-80 transition-colors">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="bg-card border border-border rounded-2xl p-8 space-y-5" data-testid="apply-form">
                  ...
                  <div className="flex gap-4">
                    {[{ value: "regular", label: "Regular (Weekday)" }, { value: "weekend", label: "Weekend" }].map(opt => (
                      <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                        <input data-testid={`radio-${opt.value}`} type="radio" value={opt.value} {...form.register("classType")} className="accent-brand-primary" />
                ...
                <button data-testid="button-submit" type="submit" disabled={createApplication.isPending} className="w-full bg-brand-primary text-white font-bold py-3.5 rounded-lg hover:opacity-80 transition-colors disabled:opacity-50">
          ...
          <div className="space-y-5">
            <div className="bg-brand-primary rounded-xl p-6 text-white">
              <h3 className="font-bold text-brand-accent mb-4">Admission Help</h3>
              <ul className="space-y-3">
                {[{ icon: Phone, text: cfgPhone }, { icon: Mail, text: cfgEmail }, { icon: MapPin, text: cfgLocation }].map(({ icon: Icon, text }) => (
              ...
              <ol className="space-y-3">
                {["Submit online application", "Receive confirmation call", "Document verification", "Fee payment & enrollment", "Get your admission letter"].map((step, i) => (
                  <li key={step} className="flex gap-3 text-sm text-muted-foreground">
                    <span className="h-6 w-6 bg-brand-primary text-white rounded-full flex items-center justify-center text-xs font-bold shrink-0">{i + 1}</span>
                    {step}
                  </li>
                ))}
              </ol>
