...
export default function Team() {
  ...
  const departments = (config?.team && config.team.length > 0)
    ? config.team.map(d => ({ name: d.department, members: d.members }))
    : defaultDepartments;
  return (
          ...
          {departments.map(dept => (
            <div key={dept.name}>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {dept.members.filter(m => m.name?.trim()).map((member, i) => {
                  const photo = (member as { photoUrl?: string }).photoUrl;
                  const initial = (member.name.split(" ").filter(w => !["Dr.", "Prof."].includes(w))[0]?.[0] ?? member.name[0] ?? "?").toUpperCase();
                  return (
                    <div key={`${member.name}-${i}`} className="bg-card border border-border rounded-xl p-5 text-center">
                      {photo ? (
                        <img src={photo} alt={member.name} className="h-16 w-16 mx-auto rounded-full object-cover mb-3" />
                      ) : (
                        <div className="h-16 w-16 mx-auto bg-brand-primary text-white rounded-full flex items-center justify-center text-xl font-bold mb-3">
                          {initial}
                        </div>
                      )}
