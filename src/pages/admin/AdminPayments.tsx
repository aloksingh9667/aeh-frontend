...
export default function AdminPayments() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-brand-primary text-white px-6 py-4 flex items-center gap-3">
        ...
        <span className="ml-auto text-sm text-white/70">Total Collected: <span className="font-bold text-brand-accent">₹{totalSuccess.toLocaleString("en-IN")}</span></span>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by student name, roll number, or receipt..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary" />
          </div>
        </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-brand-primary font-semibold">{p.receiptNumber}</td>
