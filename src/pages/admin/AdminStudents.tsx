...
import { Users, Trash2, Search, ChevronLeft, CheckCircle, XCircle, User, BookOpen, Clock, UserCheck, Calendar } from "lucide-react";
...
export default function AdminStudents() {
  ...
  const [batchEditId, setBatchEditId] = useState<number | null>(null);
  ...
  const handleSaveBatch = async (id: number) => {
    const value = batchEditValue.trim();
    if (value && !/^\d{4}-\d{2}(-\d{2})?$/.test(value)) {
      setMsg({ type: "error", text: "Use format YYYY-MM (e.g. 2026-07) or YYYY-MM-DD" });
      setTimeout(() => setMsg(null), 3000);
      return;
    }
    const r = await fetch(`${API_BASE}/students/${id}`, {
      method: "PATCH", headers,
      body: JSON.stringify({ batchStartMonth: value || null }),
    });
    if (r.ok) {
      setMsg({ type: "success", text: "Batch start month updated" });
      setBatchEditId(null);
      load(search, statusFilter);
    } else {
      setMsg({ type: "error", text: "Failed to update batch start month" });
    }
    setTimeout(() => setMsg(null), 3000);
  };
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, email or roll number..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary" />
          </div>
          <button type="submit" className="bg-brand-primary text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-80 transition-colors">Search</button>
        </form>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-gray-700">Batch Start</th>
                    ...
                    <tr key={s.id} className={`hover:bg-gray-50 transition-colors ${s.status === "pending" ? "bg-amber-50/50" : ""}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${s.status === "pending" ? "bg-amber-100" : "bg-brand-primary/10"}`}>
                            {s.status === "pending"
                      ...
                      <td className="px-4 py-3">
                        {batchEditId === s.id ? (
                          <div className="flex items-center gap-1">
                            ...
                            />
                            <button onClick={() => handleSaveBatch(s.id)} className="text-xs text-green-600 font-medium">Save</button>
                            <button onClick={() => setBatchEditId(null)} className="text-xs text-gray-400">✕</button>
                          </div>
                        ) : (
                          <button
                            onClick={() => { setBatchEditId(s.id); setBatchEditValue(s.batchStartMonth || ""); }}
                            ...
                            title="Click to edit batch start month — used for fee period calculation"
                          >
                            <Calendar className="h-3 w-3" />
                            {s.batchStartMonth || <span className="text-gray-400 italic">Not set</span>}
                          </button>
                        )}
                      </td>
                      <td className="px-4 py-3">
