...
import { z } from "zod";
import { useAdminLogin } from "@/lib/api-client/index";
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center h-16 w-16 bg-brand-accent rounded-2xl mb-4">
            <GraduationCap className="h-9 w-9 text-brand-primary" />
          </div>
          <h1 className="text-2xl font-bold text-white">Admin Portal</h1>
          <p className="text-white/60 mt-1 text-sm">Manage your institution</p>
        </div>
        <form onSubmit={form.handleSubmit(onSubmit)} className="bg-white rounded-2xl p-8 shadow-2xl space-y-5" data-testid="login-form">
          ...
          <button data-testid="button-login" type="submit" disabled={loginMutation.isPending} className="w-full bg-brand-primary text-white font-bold py-3 rounded-lg hover:opacity-80 transition-colors disabled:opacity-50">
