...
export default function AdminCreate() {
  ...
  return (
    <div className="min-h-screen bg-brand-primary flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center h-16 w-16 bg-brand-accent rounded-2xl mb-4">
            <ShieldCheck className="h-9 w-9 text-brand-primary" />
          </div>
          <h1 className="text-2xl font-bold text-white">Create New Admin</h1>
          <p className="text-white/60 mt-1 text-sm">Admin Management</p>
        </div>
          ...
          {(["secret", "otp", "details"] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                step === s ? "bg-brand-accent text-brand-primary" :
        ...
        <div className="bg-white rounded-2xl p-8 shadow-2xl">
          ...
          {step === "secret" && (
            <form onSubmit={handleVerifySecret} className="space-y-5">
              <div className="text-center mb-4">
                <Lock className="h-8 w-8 text-brand-primary mx-auto mb-2" />
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary"
                />
              </div>
              <button type="submit" disabled={loading} className="w-full bg-brand-primary text-white font-bold py-3 rounded-lg hover:opacity-80 transition-colors disabled:opacity-50">
          ...
          {step === "otp" && (
            <form onSubmit={handleVerifyOtp} className="space-y-5">
              <div className="text-center mb-4">
                <KeyRound className="h-8 w-8 text-brand-primary mx-auto mb-2" />
              ...
              <div>
                ...
                <input
                  ...
                  required
                  maxLength={6}
                  className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary tracking-widest text-center text-lg font-mono"
                />
              </div>
              <button type="submit" disabled={loading} className="w-full bg-brand-primary text-white font-bold py-3 rounded-lg hover:opacity-80 transition-colors disabled:opacity-50">
          ...
          {step === "details" && (
            <form onSubmit={handleCreateAdmin} className="space-y-4">
              <div className="text-center mb-4">
                <User className="h-8 w-8 text-brand-primary mx-auto mb-2" />
              ...
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required placeholder="e.g. John Doe" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Username</label>
                <input type="text" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} required placeholder="e.g. john.admin" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Password</label>
                <input type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} required placeholder="Minimum 6 characters" className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Role</label>
                <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary">
                  {ROLES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                </select>
              </div>
              <button type="submit" disabled={loading} className="w-full bg-brand-primary text-white font-bold py-3 rounded-lg hover:opacity-80 transition-colors disabled:opacity-50 mt-2">
          ...
          {step === "done" && createdAdmin && (
            <div className="text-center space-y-4">
              ...
              <button onClick={() => setLocation("/admin/login")} className="w-full bg-brand-primary text-white font-bold py-3 rounded-lg hover:opacity-80 transition-colors">
                Go to Admin Login
              </button>
            </div>
          )}
