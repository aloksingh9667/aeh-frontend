...
export function SchoolPage({ name, tagline, intro, color, highlights, programs, whyChoose, careers, image, stats }: SchoolPageProps) {
  ...
  const phone = config?.phone || "+91 77721 56789";
  const email = config?.email || "admissions@college.edu.in";
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-10">
            ...
            <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <Link href="/apply" className="inline-flex items-center gap-2 text-brand-primary font-semibold text-sm hover:gap-3 transition-all">
            ...
            <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                {careers.map(c => (
                  <span key={c} className="bg-brand-primary/8 text-brand-primary border border-brand-primary/20 text-sm px-4 py-2 rounded-full font-medium hover:bg-brand-primary hover:text-white transition-colors cursor-default">
                    {c}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-brand-primary rounded-2xl p-6 text-white">
              <h3 className="font-bold text-brand-accent text-base mb-4">Why Choose {collegeName}?</h3>
              <ul className="space-y-3">
                {whyChoose.map(w => (
                  <li key={w} className="flex items-start gap-2.5 text-sm text-white/80">
                    <div className="h-5 w-5 bg-brand-accent/20 rounded-full flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
              <div className="text-center mb-4">
                ...
                <p className="text-gray-500 text-sm mt-1">Apply online and start your journey at {collegeName}</p>
              </div>
              <Link href="/apply" className="block bg-brand-primary text-white text-center font-bold py-3 rounded-xl hover:opacity-80 transition-colors mb-3">
                Apply Online — Free
              </Link>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
              ...
              <a href={`tel:${phone.replace(/\s/g, "")}`} className="block text-amber-900 font-bold text-lg mb-1">{phone}</a>
              <a href={`mailto:${email}`} className="text-amber-700 text-sm hover:underline">{email}</a>
