...
import { z } from "zod";
import { useCreateContact } from "@/lib/api-client/index";
...
export default function Contact() {
  ...
  const cfgEmail = config?.email || "info@college.edu.in";
  const cfgCollegeName = config?.collegeName || "Our College";
  const cfgAddress = [config?.address, config?.city, config?.state, config?.pincode].filter(Boolean).join(", ") || "Your City, State";
  const cfgHours = config?.officeHours || "Mon-Sat: 9:00 AM - 6:00 PM";
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            {submitted ? (
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="bg-card border border-border rounded-2xl p-8 space-y-5" data-testid="contact-form">
                ...
                <button data-testid="button-submit" type="submit" disabled={createContact.isPending} className="w-full bg-brand-primary text-white font-bold py-3.5 rounded-lg hover:opacity-80 transition-colors disabled:opacity-50">
          ...
          <div className="space-y-5">
            <div className="bg-brand-primary rounded-xl p-6 text-white">
              <h3 className="font-bold text-brand-accent mb-5">Contact Information</h3>
              <ul className="space-y-4">
                <li className="flex gap-3">
                  <MapPin className="h-5 w-5 text-brand-accent shrink-0 mt-0.5" />
                  <div>
                    <div className="font-medium text-sm">Address</div>
                    <div className="text-white/70 text-sm mt-0.5">{cfgCollegeName},<br />{cfgAddress}</div>
                  </div>
                </li>
                <li className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white/70 text-sm mt-0.5">{cfgPhone}</div>
                  </div>
                </li>
                <li className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white/70 text-sm mt-0.5">{cfgEmail}</div>
                  </div>
                </li>
                <li className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white/70 text-sm mt-0.5">{cfgHours}</div>
                  </div>
                </li>
              </ul>
            </div>
