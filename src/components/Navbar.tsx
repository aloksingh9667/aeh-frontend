...
import { useSiteConfig, isFlagEnabled } from "@/lib/siteConfig";
...
export function Navbar() {
  ...
  const brandShort = config?.shortName || "AEH";
  const brandLong = config?.collegeName || "Our College";
  const dropdownRef = useRef<HTMLDivElement>(null);

  const showTopBar = isFlagEnabled(config?.featureFlags, "showTopBar", true);
  const topBarPhone = config?.topBar?.phone || config?.phone || "+91 77721 56789";
  const topBarEmail = config?.topBar?.email || config?.email || "info@college.edu.in";
  const topBarBadge = config?.topBar?.badge || "Admissions Open 2026-27";
  const topBarDeadline = config?.topBar?.deadline || "Last Date: June 30, 2026";

  const showApply = config?.navbar?.showApplyButton !== false;
  const showStudentLogin = config?.navbar?.showStudentLogin !== false;
  const applyText = config?.navbar?.applyButtonText || "Apply Now";

  const customNavItems = (config?.navbar?.items || []).filter(i => i.visible !== false && i.label && (i.href || i.children?.length));
  const useCustomMenu = customNavItems.length > 0;
  ...
  return (
    <>
      {/* Top Info Bar */}
      {showTopBar && (
        <div className="text-white/60 text-xs px-4 hidden md:block" style={{ backgroundColor: "color-mix(in srgb, var(--brand-primary) 80%, #000)" }}>
          <div className="max-w-7xl mx-auto flex items-center justify-between h-8">
            <div className="flex items-center gap-6">
              <a href={`tel:${topBarPhone.replace(/\s+/g, "")}`} className="flex items-center gap-1.5 hover:text-white transition-colors">
                <Phone className="h-3 w-3" /> {topBarPhone}
              </a>
              <a href={`mailto:${topBarEmail}`} className="flex items-center gap-1.5 hover:text-white transition-colors">
                <Mail className="h-3 w-3" /> {topBarEmail}
              </a>
            </div>
            <div className="flex items-center gap-4 text-white/50">
              <span>{topBarBadge}</span>
              {topBarDeadline && <span className="font-medium" style={{ color: "var(--brand-accent)" }}>{topBarDeadline}</span>}
            </div>
          </div>
        </div>
      )}
      ...
      <nav
        ref={dropdownRef}
        className={`text-white sticky top-0 z-50 transition-shadow duration-200 ${scrolled ? "shadow-2xl" : "shadow-lg"}`}
      ...
      >
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="flex items-center gap-2.5 font-bold shrink-0 group">
              <div className="h-9 w-9 rounded-lg flex items-center justify-center group-hover:scale-105 transition-transform overflow-hidden" style={{ backgroundColor: "var(--brand-accent)" }}>
                {config?.logoUrl ? (
                  <img src={config.logoUrl} alt={brandShort} className="h-full w-full object-cover" />
                ) : (
                  <GraduationCap className="h-5 w-5" style={{ color: "var(--brand-primary)" }} />
                )}
              </div>
              <div className="leading-tight">
                <span className="font-extrabold text-base hidden sm:block" style={{ color: "var(--brand-accent)" }}>{brandShort}</span>
                <span className="text-white/80 text-xs font-medium hidden sm:block truncate max-w-[200px]">{brandLong}</span>
                <span className="font-extrabold text-base sm:hidden" style={{ color: "var(--brand-accent)" }}>{brandShort}</span>
              </div>
            </Link>
                        ...
                        {item.children.length > 5 ? (
                          <div className="grid grid-cols-2 gap-0.5 p-1">
                            {item.children.map(child => (
                              ...
                              >
                                <span className="text-sm text-gray-800 font-medium group-hover:text-[var(--brand-primary)]">{child.label}</span>
                          ...
                          item.children.map(child => (
                            <DropdownItem key={child.href} href={child.href} label={child.label} onClick={() => setOpenDropdown(null)} />
                          ))
                        )}
                      </DropdownNav>
                    ) : (
                      <NavLink key={`${item.label}-${item.href}`} href={item.href} active={location === item.href}>
                        {item.label}
                      </NavLink>
                    )
                  ))}
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 gap-0.5 p-1">
                      {programs.map(l => (
                        <Link
                          ...
                          className="flex flex-col px-4 py-2.5 rounded-lg hover:bg-blue-50 group transition-colors"
                        >
                          <span className="text-sm text-gray-800 font-medium group-hover:text-[var(--brand-primary)]">{l.label}</span>
                  ...
                  <span className="xl:hidden">
                    <DropdownNav label="More" isOpen={openDropdown === "more"} onToggle={() => toggle("more")} icon={<MoreHorizontal className="h-3.5 w-3.5" />}>
                      {moreLinksDefault.map(l => <DropdownItem key={l.href} href={l.href} label={l.label} onClick={() => setOpenDropdown(null)} />)}
                    </DropdownNav>
                  </span>
                </>
              )}
            ...
            <div className="hidden lg:flex items-center gap-2 ml-1">
                ...
                <div className="relative">
                  ...
                  {openDropdown === "student" && (
                    <div className="absolute right-0 top-full mt-2 bg-white shadow-xl rounded-xl border border-gray-100 min-w-[200px] py-2 z-50">
                      ...
                      <Link href="/student/dashboard" onClick={() => setOpenDropdown(null)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50" style={{ "--hover-color": "var(--brand-primary)" } as React.CSSProperties}>
                        <User className="h-4 w-4" /> My Dashboard
                      </Link>
                      <Link href="/student/fees" onClick={() => setOpenDropdown(null)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50">
                        💳 Fee Payment
                      </Link>
                      <Link href="/student/receipts" onClick={() => setOpenDropdown(null)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50">
                        🧾 My Receipts
                      </Link>
              ...
              {showApply && (
                <Link href="/apply" className="font-bold px-4 py-2 rounded-lg text-sm whitespace-nowrap transition-opacity hover:opacity-90" style={{ backgroundColor: "var(--brand-accent)", color: "var(--brand-primary)" }}>
                  {applyText}
                </Link>
              )}
            </div>
        ...
        {/* Mobile Menu */}
        {mobileOpen && (
          <div className="lg:hidden border-t border-white/10" style={{ backgroundColor: "color-mix(in srgb, var(--brand-primary) 75%, #000)" }}>
            <div className="max-h-[80vh] overflow-y-auto">
              ...
              <div className="px-3 py-2 space-y-0.5">
                {useCustomMenu ? (
                  customNavItems.map(item => (
                    item.children && item.children.length > 0 ? (
                      <MobileSection key={item.label} label={item.label} isOpen={mobileSection === item.label} onToggle={() => setMobileSection(p => p === item.label ? null : item.label)}>
                        {item.children.map(child => (
                          <MobileNavLink key={child.href} href={child.href} onClick={closeMobile} indent>
                            {child.label}{child.sub ? <span className="text-white/30 text-xs ml-2">{child.sub}</span> : null}
                          </MobileNavLink>
                        ))}
                      </MobileSection>
                    ) : (
                      <MobileNavLink key={`${item.label}-${item.href}`} href={item.href} onClick={closeMobile}>
                        {item.label}
                      </MobileNavLink>
                    )
                  ))
              ...
              <div className="px-4 py-3 border-t border-white/10 space-y-2">
                {student ? (
                  ...
                  showStudentLogin && (
                    <Link href="/student/login" onClick={closeMobile} className="flex items-center justify-center gap-2 py-2.5 text-sm border rounded-lg hover:opacity-90 transition-opacity" style={{ color: "var(--brand-accent)", borderColor: "var(--brand-accent)" }}>
                      <User className="h-4 w-4" /> Student Login
                    </Link>
                  )
                )}
                {showApply && (
                  <Link href="/apply" onClick={closeMobile} className="block text-center font-bold px-4 py-3 rounded-lg text-sm" style={{ backgroundColor: "var(--brand-accent)", color: "var(--brand-primary)" }}>
                    {applyText} — {topBarBadge}
                  </Link>
                )}
              </div>
            </div>
...
function DropdownItem({ href, label, onClick }: { href: string; label: string; onClick: () => void }) {
  return (
    <Link href={href} onClick={onClick} className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 transition-colors" style={{ "--tw-text-opacity": "1" } as React.CSSProperties}>
      {label}
    </Link>
  );
}
...
function MobileSection({ label, isOpen, onToggle, children }: { label: string; isOpen: boolean; onToggle: () => void; children: React.ReactNode }) {
  return (
    <div>
      ...
      {isOpen && (
        <div className="ml-3 mt-0.5 border-l-2 border-white/20 space-y-0.5" style={{ borderColor: "color-mix(in srgb, var(--brand-accent) 40%, transparent)" }}>
          {children}
        </div>
      )}
    </div>
